...
void main() {
  group('ApiService Tests', () {
    ...
    group('getCategories', () {
      test('deve retornar lista de categorias com sucesso', () async {
        // Arrange
        final mockResponse = Response(
          data: ['electronics', 'jewelery', 'men\'s clothing', 'women\'s clothing'],
          statusCode: 200,
          requestOptions: RequestOptions(path: '${AppConstants.productsEndpoint}/categories'),
        );

        when(mockDio.get('${AppConstants.productsEndpoint}/categories'))
            .thenAnswer((_) async => mockResponse);
      ...
      test('deve lançar exceção em caso de erro', () async {
        // Arrange
        when(mockDio.get('${AppConstants.productsEndpoint}/categories'))
            .thenThrow(DioException(
              requestOptions: RequestOptions(path: '${AppConstants.productsEndpoint}/categories'),
    ...
    group('getProductsByCategory', () {
      test('deve retornar produtos de categoria específica', () async {
        // Arrange
        final mockResponse = Response(
          data: [
            {
              'id': 1,
              'title': 'Produto Electronics',
              'price': 99.99,
          ...
          statusCode: 200,
          requestOptions: RequestOptions(path: '${AppConstants.productsEndpoint}/category/electronics'),
        );

        when(mockDio.get('${AppConstants.productsEndpoint}/category/electronics'))
            .thenAnswer((_) async => mockResponse);
        ...
        expect(products.first.title, equals('Produto Electronics'));
      });
    });
  });
}